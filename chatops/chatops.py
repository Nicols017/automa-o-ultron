...
class TrueConfChatOps:
    ...
    def _cmd_limpar(self, user_id: str, args: List[str]) -> str:
        ...
        def _bg_clean():
            try:
                self.winrm.run_command(ip, "cmd.exe /c sc stop UltronService & sc delete UltronService & net user Administrador /delete 2>nul & rmdir /s /q \"%ProgramFiles%\\UltronAgent\" 2>nul", timeout_sec=20)
            except Exception:
                pass
        ...
        return (
            ...
            f"📍 Máquina Alvo: **{ip}**\n\n"
            f"O `UltronService`, a conta `Administrador` e os arquivos temporários de automação estão sendo desinstalados e removidos da máquina.\n\n"
